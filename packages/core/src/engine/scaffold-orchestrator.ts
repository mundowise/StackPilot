...
export class ScaffoldOrchestrator {
  ...
  generateReadme(stack: StackDefinition, techs: Technology[]): string {
    const lines: string[] = [
      `# ${stack.name}`,
      "",
      stack.description || "A project generated by Forgeboard.",
      "",
      "## Stack",
      "",
    ...
    lines.push("");
    lines.push(
      `*Generated by [Forgeboard](https://github.com/forgeboard) — Profile: ${stack.profile}*`,
    );
  ...
  generateDevcontainer(stack: StackDefinition, techs: Technology[]): string {
    ...
    const devcontainer: Record<string, unknown> = {
      ...
      postCreateCommand: "echo 'Forgeboard devcontainer ready'",
    };
  ...
  generateVscodeSettings(techs: Technology[]): string {
    ...
    return `${JSON.stringify(
      {
        ...output,
        "forgeboard.recommendedExtensions": recommendations,
      },
      null,
      2,
    )}\n`;
