...
function runCmd(cmd: string, cwd: string, timeoutMs = 120_000): { success: boolean; output: string } {
  try {
...
function scaffoldFrontend(tech: Technology, frontendDir: string, parentDir: string, projectName: string): string[] {
  const log: string[] = [];
...
function scaffoldBackend(tech: Technology, backendDir: string, _parentDir: string, projectName: string, allTechs: Technology[]): string[] {
  const log: string[] = [];
    ...
    if (mode === "template") {
      ...
      stackName = await input({
        ...
        validate: (v) => {
          ...
          if (!/^[a-zA-Z0-9_.-]+$/.test(v.trim())) return "Only letters, numbers, hyphens, dots, and underscores";
          return true;
        },
      });
