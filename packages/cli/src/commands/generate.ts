/**
 * forgeboard generate — Smart project generator.
...
import type { StackProfile, StackTechnology, Technology } from "@forgeboard/core";
import { installTechnologies } from "@forgeboard/core";
...
export const generateCommand = new Command("generate")
  ...
  .action((opts) => {
    ...
    if (unknownTechs.length > 0) {
      ...
      console.error(chalk.dim("  Run `forgeboard browse` to see available technologies."));
      process.exit(1);
    }
    ...
    if (!opts.json) {
      console.log(`\n  ${gradientHeader("Forgeboard")} ${chalk.dim("/ Generate Project")}\n`);
    }
    ...
    const { stack, validation } = engine.create({
      name: projectName,
      description: "Generated by Forgeboard",
